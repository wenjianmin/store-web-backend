...
export class UserService {
  ...
  async registry(createUserDto: CreateUserDto) {
    ...
    // 2. 存在则返回错误信息
    if (user) {
      throw new HttpException('用户名或注册邮箱已存在，请重新输入', HttpStatus.CONFLICT);
    }
    if (createUserDto.confirmPassword !== createUserDto.password) {
      throw new HttpException('两次输入的密码不一致，请重新输入', HttpStatus.EXPECTATION_FAILED);
    }
    // 3. 校验注册验证码
    const codeRedisKey = getRedisKey(RedisKeyPrefix.REGISTRY_CODE, createUserDto.email)
    const code = await this.redisService.get(codeRedisKey)
    if (!code) {
      throw new HttpException('验证码有误或已过期', HttpStatus.EXPECTATION_FAILED);
    }
    // 3. 哈希加密
    const salt = await genSalt()
    createUserDto.password = await hash(createUserDto.password, salt);

    const newUser = plainToClass(UserEntity, { salt, ...createUserDto }, {ignoreDecorators: true});
    // 3. 不存在则创建用户
  ...
  async getUserList(dto: UserListDto) {
    ...
    let queryBuilder = this.dataSource
      ...
      .select(['u.*', "JSON_ARRAYAGG(JSON_OBJECT('id', r.id, 'name', r.name)) as roles"])
      .groupBy('u.id')

      // 如果有模糊检索条件
      if (username) {
        queryBuilder = queryBuilder.where('u.username Like :username').setParameter('username', `%${username}%`)
      }
  ...
  async getCurrentUser(currentUser: UserEntity) {
    ...
    const queryBuilder = this.dataSource
      ...
      .leftJoin('store_role_permission', 'rolePerm', 'role.id = rolePerm.roleId')
  ...
  async updatePassword(dto: ForgotUserDto) {
    ...
    if (password !== confirmPassword) {
      throw new HttpException('两次输入的密码不一致', HttpStatus.EXPECTATION_FAILED);
    }
    // 1. 判断用户是否存在
    const exists = await this.userRepository.findOneBy({ username })
    if (!exists) {
      throw new HttpException('用户不存在或已删除', HttpStatus.EXPECTATION_FAILED);
    }
    const { id, freezed } = exists
    if (freezed) {
      throw new HttpException('用户已被冻结，请解冻后再修改', HttpStatus.EXPECTATION_FAILED);
    }
    ...
    if (cacheCode != code) {
      throw new HttpException('验证码错误或已过期', HttpStatus.EXPECTATION_FAILED);
    }
    // 3. 更新密码
    const newPassword = await hash(password, exists.salt)
    const { affected } = await this.userRepository.update({ id }, { password: newPassword });
    if (!affected) {
      throw new HttpException('修改失败，请稍后重试', HttpStatus.EXPECTATION_FAILED);
    }
  ...
  async sendCode(email: string) {
    ...
    if (!user) {
      throw new HttpException('请输入用户注册时填写的邮箱地址', HttpStatus.EXPECTATION_FAILED);
    }
    // 2. 发送邮件
